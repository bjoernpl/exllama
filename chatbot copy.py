...
parser.add_argument("-nnl", "--no_newline", action = "store_false", help = "Do not break bot's response on newline (allow multi-paragraph responses)")
...
class Chatbot:
    ...
    def chat(self, prompt):
        ...
        if self.first_round and args.botfirst: in_tokens = res_tokens
        ...
        if self.generator.gen_num_tokens() >= max_tokens:
            self.generator.gen_prune_to(config.max_seq_len - expect_tokens - extra_prune, self.tokenizer.newline_token_id)
        ...
        for i in range(max_response_tokens):
            ...
            yield new_text  # (character streaming output is here)
            sys.stdout.flush()

            # End conditions

            if break_on_newline and gen_token.item() == self.tokenizer.newline_token_id: break
            ...
            if res_line.endswith(f"{username}:"):
                plen = self.tokenizer.encode(f"{username}:").shape[-1]
